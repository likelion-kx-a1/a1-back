...
> 프론트엔드(Next.js)와 백엔드(Spring Boot) 양쪽의 DDD + 계층형 하이브리드 패키지 구조를 설계시킬 때 사용한 프롬프트 원본입니다. 실제 코드/문서를 만들기 위한 도구이며, `docs_h/`의 실무 기술 문서와는 별개로 이 폴더(`docs_p/`)에 격리 보관합니다.

## 1. 프론트엔드 (Next.js) 구조 설계 프롬프트
...
- **Infrastructure의 추상화**: 외부 API(Gemini, fal.ai, S3)와 결합하는 서비스는 반드시 `application` 계층에 인터페이스를 두고, 구현체만 `infrastructure`에 위치시킵니다. 이 구조가 API 키 없이 개발을 가능하게 만드는 핵심입니다.

> 참고: 이 프롬프트로 초기 스캐폴딩한 이후 실제 적용 결과는 `docs_h/아키텍처_가이드.md`를 참고하세요. 실제 구현에서는 헥사고날 Port/Adapter 이중 구조를 제거하는 등 추가 단순화가 이루어졌습니다.